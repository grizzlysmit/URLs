...

{
    ...
    sub main {
        ...
        my $dbserver        = $cfg->val('dbserver');
        my $dbuser          = $cfg->val('dbuser');
        my $dbpass          = $cfg->val('dbpass');
        my $dbname          = $cfg->val('dbname');
        my $dbport          = $cfg->val('dbport');
        my $db              = DBI->connect("dbi:mysql:database=$dbname;host=$dbserver;port=$dbport;", "$dbuser", "$dbpass", {'RaiseError' => 1});
